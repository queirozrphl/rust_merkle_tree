...
struct MerkleTree {
    nodes: Vec<Vec<u8>>, // Armazena os hashes de todos os nós
}
...
impl MerkleTree {
    ...
    fn generate_proof(&self, data_index: u8) -> Vec<(Vec<u8>, bool)> {
...
pub fn run_demo() {
    ...
    let proof = merkle_tree.generate_proof(data_index);
